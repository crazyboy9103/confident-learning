...
task_names=("chocoball")
data_paths=("chocoball")
        ...
        for epoch in "${epoch_values[@]}"; do
            for noise in "${noise_types[@]}"; do
                for i in $(seq 0 $((task_count - 1))); do
                    ...
                    python main_det.py model.optimizer.lr=$lr model.scheduler.eta_min=$eta_min num_epochs=$epoch task_name=$task paths.data_root_dir="/datasets/conflearn/det/$path" conflearn=$noise
                done
            done
        done
    done
