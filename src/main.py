...
def main(cfg: DictConfig):
    ...
    for k in range(num_folds):
        ...
        images = torch.cat([result[0] for result in preds], dim=0)
        targets = torch.cat([result[1] for result in preds], dim=0)
        probs = torch.cat([result[2] for result in preds], dim=0)
